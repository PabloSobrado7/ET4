...
function comprobarVacio(campo){
	if((campo.value == null) || (campo.value.length == 0)){ //COMPROBAMOS SI EN EL CAMPO SE HAN INTRODUCIDO VALORES.
		if(!estado){
...
function comprobarTexto(campo, size){
	if(campo.value.length > size){ //primero comprobamos que el valor del dato introducido no supera el valor que permite el campo.
		if(!estado){
...
function comprobarCorreo(campo){
	...
	if(eregular.test(campo.value) == false){ //comprobamos que los datos introducidos coincidan con el formato que estable la expresion regular.
		if(!estado){
...
function comprobarAlfabetico(campo, size){
	...
	if(alfa.test(campo.value) == false && campo.value.length <= size){ //comprobamos que los datos introducidos coincidan con el formato que estable la expresion regular.
		if(!estado){
...
function comprobarReal(campo, numD, vmenor, vmayor){
	...
	if(num < vmenor || num > vmayor){ //Comprobamos que el valor se encuentra dentro de los rangos que establece el campo
...
function comprobarDNI(campo){
	...
	var eregular_dni = /^[0-9]{8}[a-z]$/i; //Asignamos una expresion regular de dni a una variable

	if(dni.test(campo.value) == false){ //comprobamos que el campo cumple las condiciones.
		if(!estado){
		...
		if(correcta !== letra.toUpperCase(letraDni)){
...
function comprobarNumerico(campo){
	...
	if(!eregular_num.test(campo.value)){ //comprobamos que los datos en el campo cumplen las condiciones que establece la expresion regular.
		if(!estado){
